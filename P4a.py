...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    total=0
    displayHand(hand)
    player_input = raw_input("Enter word, or a \".\" to indicate that you are finished:")
    if player_input == '.':
        print "Goodbye! Total score: "+str(total)+ "points."    
    else:
        while player_input != '.':
            if isValidWord(player_input, hand, wordList):
                hand = updateHand(hand,player_input)
                score = getWordScore(player_input, n)
                total = total + score
                print player_input+" earned "+str(score)+'points. Total: '+str(total)+' points'
            else:
